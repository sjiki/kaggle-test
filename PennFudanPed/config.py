...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.05, nms=dict(type='nms', iou_thr=0.3), max_per_img=700),
    keep_all_stages=False)

# dataset settings
dataset_type = 'CustomDataset'
data_root = '../kaggle/PennFudanPed/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'dtrainval.pkl',
...
# optimizer
optimizer = dict(
    type='SGD', lr=0.02 / 16 * 2, momentum=0.9, weight_decay=0.0001)
